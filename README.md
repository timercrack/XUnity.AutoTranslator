...
- ALT + I: Toggle the UI Path Inspector overlay. Shows the hovered text component path under the mouse and logs the current selection when you click.
...
- LLMTranslate. Uses an OpenAI-compatible Chat Completions endpoint directly from inside AutoTranslator.
  - Requires a URL and model name. The API key is optional depending on your provider.
  - The configured `SystemPrompt` must make the model return either a JSON array of translated strings or an object with a `translations` array, and the number of returned items must exactly match the batch size.
  - Example Configuration:
    - Endpoint=LLMTranslate
    - [LLMTranslate]
    - Url=https://api.openai.com/v1/chat/completions
    - Model=gpt-4.1-mini
    - SystemPrompt=Translate the user's JSON array of game text into English. Return JSON array only.
...
IgnoreTextRegexes=               ;Indicates that the plugin should ignore texts matching one or more regular expressions. This is a list separated by ';'.
...
EnableUIPathInspector=False      ;Shows a floating overlay with the hovered GameObject-backed text component path. Useful for debugging GameLogTextPaths, blacklist paths, and resizer rules.
PeriodicManualHookIntervalSeconds=0 ;When above 0, traverses the scene at this interval to catch text components missed by setter hooks. Useful for transient HUD notifications.
...
TemplateAllNumberAway=False      ;Indicates whether numeric fragments should be templated into placeholders so changing values like weight, temperature, pressure, etc. can reuse the same translation.
...
[LLMTranslate]
Url=                             ;OPTIONAL, required if LLMTranslate is configured
ApiKey=                          ;OPTIONAL, depends on your provider if LLMTranslate is configured
Model=                           ;OPTIONAL, required if LLMTranslate is configured
SystemPrompt=                    ;OPTIONAL, but should instruct the model to return JSON only
Temperature=0                    ;OPTIONAL, defaults to 0
MaxTokens=4096                   ;OPTIONAL, defaults to 4096
BatchSize=10                     ;OPTIONAL, max translations to send in a single LLM request
MaxConcurrency=1                 ;OPTIONAL, maximum parallel requests for this endpoint
EnableShortDelay=False           ;OPTIONAL, set translation delay to 0.1 seconds
DisableSpamChecks=False          ;OPTIONAL, disables plugin-side spam checks for this endpoint
...
There is now also a lightweight built-in helper for this: enable `[Behaviour] EnableUIPathInspector=True` or press `ALT+I` in-game. It will show the path of the hovered text component in a floating overlay and write the current selection to the log when you click. This only works for GameObject-backed text components (UGUI, TMP, TextMesh, NGUI, etc.), not for IMGUI-only content.
